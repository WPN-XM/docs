...
<5> We are working on the filtered `$node->text()` and grab the version by using a regexp for a version scheme.
    The matched value is then reassigned to `$version`.
<6> We compare the scraped version number with the latest version we have in our registry.
    When the scraped version is newer, than the latest version in the registry, we return an array as response.
