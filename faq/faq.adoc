...
You find more information in the http://wpn-xm.github.io/docs/user-manual/en/#_using_the_server_control_panel[user-manual].
...
It is not configured in a secure way. This allows the developer to start new projects fastly.
...
We are looking for contributions to our security checklist. The list should act as a starting point, 
when a developer tries to secure the server environment. It's not complete. 
You have to fix all security issues manually.

- delete the webinterface
- close the firewall 
- open only the firewall ports of the servers you really need
...
The Lite installer provide only a few essential software components to get quickly started, 
...
The main folder for your projects is the `\www`.

Please create a new folder for your project.  Example-wise: let's create a folder "myproject" 
and put a simple HTML filewith the following content in this directory: 
...
No, that's not supporte, yet. 

== What are "automatic start pages" for the `www` directory and it's subfolders?

Nginx is configured to serve the following files as index pages: `index.html`, `index.php`.
...
When those files are not found, the server will display the directory listing.
...
[IMPORTANT]
====
Send us an email to security (at) wpn-xm (dot) org, if you find any issue.
====
////
