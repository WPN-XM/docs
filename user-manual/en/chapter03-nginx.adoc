...
`server\bin\nginx\conf\sites-enabled`. This is done by `include sites-enabled/*.conf;`.

These configuration files are mostly server blocks. They contain directives to serve specific 
PHP applications,   configure proxy forwarding or define application specific URL rewrite rules, 
which are needed to enable nice looking URLs without parameters.
...
By default `nginx.conf` will include all upstream configs found (`include upstreams/*.conf;`).
